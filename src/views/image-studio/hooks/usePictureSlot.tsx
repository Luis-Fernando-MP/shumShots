...
const usePictureSlot = (pictureId: string) => {
  const picture = usePicturesStore(s => s.pictures.find(item => item.id === pictureId))
  const libraryId = picture?.libraryId ?? null
  ...
  const handleDropFile = async (files: DropzoneFile[]) => {
    const file = files[0]
    if (!file) return
  ...
  return {
    ...
    handleLoadError,
    handleDropFile,
    handleSize: (size: { width: number; height: number; aspectRatio: number }) =>
