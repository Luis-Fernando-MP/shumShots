...
const MonacoEditor: FC = () => {
  ...
  const showExplorer =
    chrome.fileExplorer && (!chrome.activityBar || activityActive === 'files')
  ...
  return (
    ...
    >
      <header
        className={cn(
          'relative flex shrink-0 items-center gap-3 px-3',
          chrome.headerAccent && 'border-b border-current/15'
        )}
        ...
        {showControls && (
          <div
            className={cn(
              ...
              !controlsOnLeft && 'order-2 ml-auto'
            )}
          >
      ...
      {chrome.statusBar && (
        <StatusBar
          ...
          density={chrome.statusBarDensity}
        />
      )}
    </article>
  )
